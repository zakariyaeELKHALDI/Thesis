...
JUDGEMENT_REUSE_KEYS = {
    ...
    "total_v3_candidate_count",
    "transfer_method",
    "transferred_values",
}

QUERY_EMBEDDING_REUSE_KEYS = {
...
NOTEBOOK_TRACEABILITY_KEYS = {
    "configured_relative_path",
    "executed_relative_path",
    "executed_sha256",
    "paths_match",
}

SOURCE_V2_KEYS = {
    "annotation_notebook_commit",
    "completed_judgement_commit",
    "completed_judgement_count",
    "completed_judgement_relative_path",
...
TARGET_V3_KEYS = {
    ...
    "configuration_relative_path",
    "configuration_sha256",
    "diagnostic_tail_end_rank",
    "diagnostic_tail_start_rank",
    "judgement_pool_depth",
    "maximum_candidate_depth",
    "protocol_amendment_commit",
    "protocol_amendment_relative_path",
    "protocol_amendment_sha256",
    "shared_configuration_blinding_list_interpretation",
    "shared_configuration_freeze_field_interpretation",
}

PROTOCOL_AMENDMENT_COMMIT = "5c8e0b934d61fa37f1e7551efed24c5f1597989a"
PROTOCOL_AMENDMENT_PATH = (
    "docs/retrieval-depth-evaluation-v3-protocol-amendment.md"
)
PROTOCOL_AMENDMENT_SHA256 = (
    "ab63e27618cf76c2b15cbb483d217895"
    "58a07a2dcbe9bb764e65b1b6762efd12"
)
BLINDING_LIST_INTERPRETATION = (
    "The inherited configuration list names retrieval-specific values. "
    "Question identifiers, chunk identifiers and source provenance also "
    "remain excluded from the annotation display under the version 3 "
    "protocol."
)
FREEZE_FIELD_INTERPRETATION = (
    "The inherited configuration-schema field records that the protocol "
    "and reused query-embedding matrix were fixed before the version 3 "
    "expanded search. Version 3 did not generate new query embeddings."
)


@dataclass(frozen=True)
class RetrievalExpansionPreparationResult:
    """Paths, fingerprints and reuse counts from v3 preparation."""
...
def load_retrieval_evaluation_v3_lineage(path: str | Path) -> dict[str, Any]:
    """Load the frozen v2-to-v3 reuse contract."""
    ...
    try:
        ...
        raise RetrievalEvaluationSchemaError(
            "Cannot read the v3 lineage contract as valid JSON."
        ) from error

    lineage = _require_mapping(raw, "V3 lineage contract")
    _require_exact_keys(lineage, LINEAGE_TOP_LEVEL_KEYS, "V3 lineage contract")
    if lineage["lineage_schema_version"] != LINEAGE_SCHEMA_VERSION:
        raise RetrievalEvaluationSchemaError("Unsupported v3 lineage schema version.")
    if lineage["decision_status"] != "frozen_before_v3_expanded_search":
        raise RetrievalEvaluationSchemaError("Unexpected v3 lineage decision status.")
    ...
    _require_exact_keys(
        lineage["judgement_reuse"],
        JUDGEMENT_REUSE_KEYS,
        "judgement_reuse",
    )
    _require_exact_keys(
        lineage["query_embedding_reuse"],
        QUERY_EMBEDDING_REUSE_KEYS,
        "query_embedding_reuse",
    )
    _require_exact_keys(lineage["source_v2"], SOURCE_V2_KEYS, "source_v2")
    _require_exact_keys(lineage["target_v3"], TARGET_V3_KEYS, "target_v3")
    notebook = _require_mapping(
        lineage["source_v2"]["notebook_traceability"],
        "source_v2.notebook_traceability",
    )
    _require_exact_keys(
        notebook,
        NOTEBOOK_TRACEABILITY_KEYS,
...
def validate_lineage_against_config(
    ...
    """Validate that the lineage and v3 configuration describe one run."""

    target = lineage["target_v3"]
    ...
    expected_target = {
        ...
        "protocol_amendment_relative_path": PROTOCOL_AMENDMENT_PATH,
    ...
    if target != expected_target:
        raise RetrievalEvaluationIntegrityError(
            "The v3 lineage target differs from the v3 configuration."
        )
    protocol_path = _resolve_project_path(
        root,
        target["protocol_amendment_relative_path"],
        "target_v3.protocol_amendment_relative_path",
    )
    _require_file_hash(
        protocol_path,
        target["protocol_amendment_sha256"],
        "Frozen v3 protocol amendment",
    )
    ...
    if (existing_start, existing_end, new_start, new_end) != (1, 30, 31, 40):
        raise RetrievalEvaluationIntegrityError("Unexpected v3 judgement rank ranges.")
    ...
    if reuse.get("total_v3_candidate_count") != expected_total:
        raise RetrievalEvaluationIntegrityError("Unexpected total v3 candidate count.")

    if matrix.get("api_request_required") is not False:
        raise RetrievalEvaluationIntegrityError("V3 must not request new query embeddings.")
...
def load_reused_query_matrix(
    ...
    *,
    ...
    """Load and validate the exact frozen v2 query matrix without normalising it again."""
...
def migrate_reused_judgements(
    ...
    *,
    ...
    """Transfer verified grades to matching v3 pairs and leave only new ranks blank."""
    ...
    for target_judgement in target_judgements:
        ...
        if rank <= reused_rank_end:
            ...
            if source_candidate is None:
                raise RetrievalEvaluationIntegrityError(
                    "A reused v3 question-chunk pair has no v2 candidate."
                )
            for field in prefix_match_fields:
                if source_candidate.get(field) != target_candidate.get(field):
                    raise RetrievalEvaluationIntegrityError(
                        f"V2 and v3 candidate prefixes differ on {field}."
                    )
            ...
            if source_judgement is None:
                raise RetrievalEvaluationIntegrityError(
                    "A reused v2 candidate has no completed judgement."
                )
            if (
            ...
            if (
                ...
                raise RetrievalEvaluationIntegrityError(
                    "A new v3 judgement is unexpectedly completed."
                )
            new_count += 1
...
def _load_source_v2_evidence(
    lineage: dict[str, Any],
    root: Path,
) -> dict[str, Any]:
    """Validate and load every frozen v2 artifact needed by v3."""

    source = lineage["source_v2"]
    matrix_contract = lineage["query_embedding_reuse"]
    notebook_contract = source["notebook_traceability"]
    path_fields = {
        "config": ("configuration_relative_path", "configuration_sha256"),
        "lineage": ("lineage_relative_path", "lineage_sha256"),
        ...
        "metrics": ("metrics_relative_path", "metrics_sha256"),
        "table": ("metrics_table_relative_path", "metrics_table_sha256"),
        "scoring_audit": (
            "scoring_audit_relative_path",
            "scoring_audit_sha256",
        ),
        "notebook": (None, "executed_sha256"),
    }
    paths: dict[str, Path] = {}
    for name, (path_key, hash_key) in path_fields.items():
        if name == "matrix":
            ...
            relative_path = notebook_contract.get("executed_relative_path")
            expected_hash = notebook_contract.get(hash_key)
        else:
            relative_path = source.get(path_key)
            expected_hash = source.get(hash_key)
        path = _resolve_project_path(root, relative_path, f"source_v2.{name}")
        _require_file_hash(path, expected_hash, f"Frozen v2 {name}")
        paths[name] = path

    source_config = load_retrieval_evaluation_config(paths["config"])
    if source_config["configuration_id"] != source.get("configuration_id"):
        raise RetrievalEvaluationIntegrityError("The v2 configuration ID differs.")
    if source.get("ranked_candidate_pool_depth") != 30:
        raise RetrievalEvaluationIntegrityError("The v2 pool depth differs.")
    if source.get("retrieval_adequacy_gate_triggered") is not True:
        raise RetrievalEvaluationIntegrityError("The v2 adequacy gate contract differs.")
    if source.get("selected_depth_k") is not None:
        raise RetrievalEvaluationIntegrityError("The v2 lineage selected a depth.")
    if notebook_contract.get("paths_match") is not False:
        raise RetrievalEvaluationIntegrityError(
            "The recorded v2 notebook-path discrepancy changed."
        )
    source_inputs = load_retrieval_evaluation_inputs(paths["config"], root)
    source_candidates = _read_json_lines(paths["pool"], "Frozen v2 candidate pool")
    _validate_candidate_pool(source_candidates, source_inputs)
    source_judgements = _read_json_lines(
        paths["judgements"],
        "Frozen v2 completed judgements",
    )
    _validate_completed_judgements(
        source_judgements,
        source_candidates,
    ...
    if len(source_judgements) != source.get("completed_judgement_count"):
        raise RetrievalEvaluationIntegrityError("The v2 completed count differs.")

    source_summary = _read_json_object(paths["summary"], "Frozen v2 summary")
    if source_summary.get("relevance_judgement_template_sha256") != source.get(
        "relevance_judgement_template_sha256"
    ):
        raise RetrievalEvaluationIntegrityError(
            "The v2 empty judgement-template fingerprint differs."
        )
    if source_summary.get("candidate_count") != source.get(
        "completed_judgement_count"
    ):
        raise RetrievalEvaluationIntegrityError(
            "The v2 preparation candidate count differs."
        )
    if source_summary.get("ranked_candidate_pool_sha256") != source.get(
        "ranked_candidate_pool_sha256"
    ):
        raise RetrievalEvaluationIntegrityError(
            "The v2 summary references a different candidate pool."
        )

    preparation_audit = _read_json_object(
        paths["preparation_audit"],
        "Frozen v2 preparation audit",
    )
    if preparation_audit.get("audit_status") != "PASSED":
        raise RetrievalEvaluationIntegrityError(
            "The v2 preparation audit did not pass."
        )
    if preparation_audit.get("v2_ranked_candidate_pool_sha256") != source.get(
        "ranked_candidate_pool_sha256"
    ):
        raise RetrievalEvaluationIntegrityError(
            "The v2 preparation audit references a different candidate pool."
        )

    completion_audit = _read_json_object(
        paths["completion_audit"],
        "Frozen v2 completion audit",
    )
    if completion_audit.get("audit_status") != "PASSED":
        raise RetrievalEvaluationIntegrityError(
            "The v2 completion audit did not pass."
        )
    if completion_audit.get("judgement_sha256") != source.get(
        "completed_judgement_sha256"
    ):
        raise RetrievalEvaluationIntegrityError(
            "The v2 completion audit references different judgements."
        )
    if completion_audit.get("remaining_judgement_count") != 0:
        raise RetrievalEvaluationIntegrityError(
            "The v2 completion audit is not complete."
        )

    source_metrics = _read_json_object(paths["metrics"], "Frozen v2 metrics")
    if source_metrics.get("candidate_range_adequacy_gate", {}).get("triggered") is not True:
        raise RetrievalEvaluationIntegrityError("The v2 adequacy gate did not trigger.")
    if source_metrics.get("selected_depth_k") is not None:
        raise RetrievalEvaluationIntegrityError("The v2 metrics selected a depth.")
    if source_metrics.get("relevance_judgements_sha256") != source.get(
        "completed_judgement_sha256"
    ):
        raise RetrievalEvaluationIntegrityError(
            "The v2 metrics reference different completed judgements."
        )
    if source_metrics.get("ranked_candidate_pool_sha256") != source.get(
        "ranked_candidate_pool_sha256"
    ):
        raise RetrievalEvaluationIntegrityError(
            "The v2 metrics reference a different candidate pool."
        )

    scoring_audit = _read_json_object(
        paths["scoring_audit"],
        "Frozen v2 scoring audit",
    )
    if scoring_audit.get("audit_status") != "PASSED":
        raise RetrievalEvaluationIntegrityError(
            "The v2 scoring audit did not pass."
        )
    if scoring_audit.get("candidate_range_adequacy_gate_triggered") is not True:
        raise RetrievalEvaluationIntegrityError(
            "The v2 scoring audit did not record the triggered gate."
        )
    if scoring_audit.get("selected_depth_k") is not None:
        raise RetrievalEvaluationIntegrityError(
            "The v2 scoring audit selected a depth."
        )
    if scoring_audit.get("metrics_sha256") != source.get("metrics_sha256"):
        raise RetrievalEvaluationIntegrityError(
            "The v2 scoring audit references different metrics."
        )
    if scoring_audit.get("metrics_table_sha256") != source.get(
        "metrics_table_sha256"
    ):
        raise RetrievalEvaluationIntegrityError(
            "The v2 scoring audit references a different metrics table."
        )
    return {
...
def prepare_retrieval_evaluation_v3(
    config_path: str | Path,
    lineage_path: str | Path,
    project_root: str | Path,
) -> RetrievalExpansionPreparationResult:
    """Prepare v3 by reusing frozen v2 evidence and searching locally to rank 40."""
    ...
    lineage = load_retrieval_evaluation_v3_lineage(resolved_lineage_path)
    ...
    if existing:
        raise RetrievalEvaluationError(
            "V3 preparation output already exists; refusing to replace it."
        )
    if paths["metrics"].exists() or paths["table"].exists():
        raise RetrievalEvaluationError(
            "V3 metric output exists before preparation."
        )
    if len(set(preparation_paths)) != len(preparation_paths):
        raise RetrievalEvaluationIntegrityError("V3 output paths are not unique.")

    source = _load_source_v2_evidence(lineage, root)
    source_paths = set(source["paths"].values())
    if any(path in source_paths for path in preparation_paths):
        raise RetrievalEvaluationIntegrityError(
            "A v3 output path overlaps a frozen v2 artifact."
        )

    inputs = load_retrieval_evaluation_inputs(resolved_config_path, root)
    if inputs.config["inputs"] != source["config"]["inputs"]:
        raise RetrievalEvaluationIntegrityError("V2 and v3 retrieval inputs differ.")
    if config["inputs"]["embedding_index"]["faiss_index_sha256"] != lineage[
        "source_v2"
    ]["faiss_index_sha256"]:
        raise RetrievalEvaluationIntegrityError("The v3 FAISS index differs from v2.")
    if config["inputs"]["evaluation_questions"]["questions_sha256"] != lineage[
        "source_v2"
    ]["question_sha256"]:
        raise RetrievalEvaluationIntegrityError("The v3 questions differ from v2.")
    ...
    with TemporaryDirectory(dir=root) as temporary_directory:
        ...
        summary = {
            ...
            "source_completed_judgements_sha256": lineage["source_v2"]["completed_judgement_sha256"],
            "source_configuration_id": lineage["source_v2"]["configuration_id"],
            "source_ranked_candidate_pool_sha256": lineage["source_v2"]["ranked_candidate_pool_sha256"],
        }
        ...
        if _read_json_lines(temporary_pool, "Staged v3 pool") != candidates:
            raise RetrievalEvaluationIntegrityError("V3 pool changed during staging.")
        if _read_json_lines(temporary_judgements, "Staged v3 judgements") != judgements:
            raise RetrievalEvaluationIntegrityError(
                "V3 judgements changed during staging."
            )
        if _read_json_object(temporary_summary, "Staged v3 summary") != summary:
            raise RetrievalEvaluationIntegrityError("V3 summary changed during staging.")
...
def _build_argument_parser() -> argparse.ArgumentParser:
    """Build the local v3 expansion command-line interface."""
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/retrieval-evaluation-config-v3.json"),
    )
    parser.add_argument(
        "--lineage",
        type=Path,
        default=Path("configs/retrieval-evaluation-v3-lineage.json"),
    )
...
def main() -> None:
    """Prepare v3 without an API call and report only safe metadata."""

    arguments = _build_argument_parser().parse_args()
    result = prepare_retrieval_evaluation_v3(
        arguments.config,
        arguments.lineage,
