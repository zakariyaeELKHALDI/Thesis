...
The production record unit and deterministic export contracts are frozen in `docs/corpus-record-and-export-schema.md`. Notebook audits verify 670 non-empty retrieval-region records, complete reconstruction of cleaned page text, unique region identifiers and valid coordinates for all 40,370 positioned lines present before the final publisher-label exclusion. The reusable production chain is implemented in `src/geotech_rag/corpus_geometry.py`, `src/geotech_rag/corpus_boundaries.py`, `src/geotech_rag/corpus_extraction.py` and `src/geotech_rag/corpus_export.py`.
...
Seven extraction-export tests verify schema conversion, coordinate and leakage failures, deterministic UTF-8 serialisation, control-character round trips and integration against the validated local source. The production run emitted 670 retrieval records and 770 page-audit records, and a controlled overwrite reproduced all three extraction files byte for byte. The derived files remain excluded from Git because they contain copyrighted source text.

## Completed chunking decision and implementation
...
The reconstructed baseline therefore uses a single-newline separator with Python `len`, `keep_separator=False`, `strip_whitespace=False` and the reported size and overlap values. A real-source integration check then found that 33 positioned lines contain embedded `U+000A` values and 10 contain embedded `U+000D` values originating from the preserved mathematical-font extraction. Raw newline splitting created boundaries inside positioned lines for five parent records. The production implementation temporarily protects embedded `U+000A` with source-absent `U+E000`, keeps character offsets unchanged and restores the original controls before output. This produced 5,113 chunks, 848,280 emitted characters and zero positioned-line boundary problems. Disabling whitespace stripping continues to preserve the three formula controls removed by the installed default. The configured ten-character overlap remains best-effort at complete-line boundaries.

Reusable chunking is implemented in `src/geotech_rag/corpus_chunking.py` and verified by nine focused tests, including integration against the exact region export. The production files contain 5,113 chunk records and one summary, with chunk-file SHA-256 `bce9702308a65b9e9ab3bcdb0781ad308d509b56904a95ec7bab4feffebca645`. An independent audit confirmed complete provenance and no sentinel leakage, and a controlled overwrite reproduced both files byte for byte. The complete automated suite now contains 42 passing tests. The full decision and schema are recorded in `docs/corpus-chunking-decision-and-schema.md`, and the machine-readable settings are stored in `configs/chunking-config.json`.
