...
from geotech_rag.retrieval_evaluation_v3 import (
    PREFIX_MATCH_FIELDS,
    load_retrieval_evaluation_v3_lineage,
...
V3_CONFIG_PATH = PROJECT_ROOT / "configs/retrieval-evaluation-config-v3.json"
V3_LINEAGE_PATH = PROJECT_ROOT / "configs/retrieval-evaluation-v3-lineage.json"
...
def _migration_material() -> tuple[list[dict], list[dict], list[dict], list[dict]]:
    """Build two reusable ranks and one new v3 rank."""
    ...
    target_candidates = [
        _candidate("v3", 1, "chunk-a", 0.9),
        _candidate("v3", 2, "chunk-b", 0.8),
        _candidate("v3", 3, "chunk-c", 0.7),
    ]
    source_judgements = [
...
def test_repository_v3_configuration_and_lineage_align() -> None:
    """Require the tracked v3 contracts to describe the same run."""

    config = load_retrieval_evaluation_config(V3_CONFIG_PATH)
    lineage = load_retrieval_evaluation_v3_lineage(V3_LINEAGE_PATH)
    ...
    assert config["search"]["judgement_pool_depth"] == 40
    assert lineage["judgement_reuse"]["reused_judgement_count"] == 600
    assert lineage["judgement_reuse"]["new_judgement_count"] == 200


def test_v3_lineage_rejects_additional_contract_fields(tmp_path: Path) -> None:
    """Keep the committed reuse contract closed to silent field additions."""

    lineage = json.loads(V3_LINEAGE_PATH.read_text(encoding="utf-8"))
    lineage["unexpected_field"] = True
    changed_path = tmp_path / "changed-lineage.json"
    ...
    with pytest.raises(
        ...
        load_retrieval_evaluation_v3_lineage(changed_path)


def test_v3_terminal_adequacy_gate_is_frozen() -> None:
    """Require the final planned expansion and its stopping action."""

    config = json.loads(V3_CONFIG_PATH.read_text(encoding="utf-8"))
    gate = config["candidate_range_adequacy_gate"]

    assert gate["trigger"] == (
        "A question without direct evidence in ranks 1-30 gains direct "
        "evidence in ranks 31-40."
    )
    assert "rather than automatically expanding" in gate["action_if_triggered"]
    assert gate["pool_depth_is_not_a_candidate_depth"] is True
...
def test_v3_output_paths_are_separate_from_v2() -> None:
    """Require every generated v3 output path to differ from v1 and v2."""

    v1_path = PROJECT_ROOT / "configs/retrieval-evaluation-config.json"
    v2_path = PROJECT_ROOT / "configs/retrieval-evaluation-config-v2.json"
    v1 = json.loads(v1_path.read_text(encoding="utf-8"))
    v2 = json.loads(v2_path.read_text(encoding="utf-8"))
    v3 = json.loads(V3_CONFIG_PATH.read_text(encoding="utf-8"))
    ...
    assert all(
        v1["outputs"][field] != v3["outputs"][field]
        and v2["outputs"][field] != v3["outputs"][field]
